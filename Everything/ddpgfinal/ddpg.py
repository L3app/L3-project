...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        ...
        weights1 = {
            'w1c': tf.Variable(tf.random_normal([1, n_hidden_1])),
            'w2c': tf.Variable(tf.random_normal([n_hidden_1, n_hidden_2])),
            'w2a': tf.Variable(tf.random_normal([1, n_hidden_2])),
            'outc': tf.Variable(tf.random_normal([n_hidden_2, 1]))
            }
        biases1 = {
            'b1c': tf.Variable(tf.random_normal([n_hidden_1])),
            'b2c': tf.Variable(tf.random_normal([n_hidden_2])),
            'outc2': tf.Variable(tf.random_normal([1]))
            }

        # 1st Hidden layer, OPTION: Softmax, relu, tanh or sigmoid
        layer1 = tf.nn.relu(tf.matmul(inputs, weights1['w1c']) + biases1['b1c'])
        # 2nd Hidden layer, OPTION: Softmax, relu, tanh or sigmoid
        # Action inserted here
        layer2 = tf.nn.relu(tf.matmul(layer1, weights1['w2c']) + tf.matmul(action, weights1['w2a']) + biases1['b2c'])

        out = tf.matmul(layer2, weights1['outc']) + biases1['outc2']
...
########################################################################################################
        print('a: ', actor.predict(np.reshape(1.0, (1, actor.s_dim))))
        actor_noise = OrnsteinUhlenbeckActionNoise(mu=np.zeros(action_dim))

        summary_ops, summary_vars = build_summaries()
        
        
        print('b: ', actor.predict(np.reshape(1.0, (1, actor.s_dim))))
        ...
        for i in range(int(50000)):
            ...
            for k in range(100):
                ...
                rate.sleep()
            time1 = timer1  
            print('c: ', actor.predict(np.reshape(1.0, (1, actor.s_dim))))  
            for j in range(int(10000)):
                ...
                print('s: ', s)
                print('d: ', actor.predict(np.reshape(s, (1, actor.s_dim))))
                #a = actor.predict(np.reshape(s, (1, actor.s_dim)))
                #stable x pid
