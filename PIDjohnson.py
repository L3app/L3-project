    ...
    while not rospy.is_shutdown():
        ...
        if stateManagerInstance.getLoopCount() > 100:
           #hover pid
            ...
            if switch == 0:
                controller.setVel([0,u2,u],[0,0,0])
                u, ui_prev, e_prev = PID(range1, 1.5, 1, 1, 1, ui_prev, e_prev, 0.5)
                u2, ui_prev2, e_prev2 = PID(ypos, 0, 1, 1, 1, ui_prev2, e_prev2, 0.5)
                xcontrol = 0   
            elif switch == 1:
                controller.setVel([xcontrol,u2,u],[0,0,0])
                u, ui_prev, e_prev = PID(range1, 1.5, 1, 1, 1, ui_prev, e_prev, 0.5)
                u2, ui_prev2, e_prev2 = PID(ypos, 0, 1, 1, 1, ui_prev2, e_prev2, 0.5)
