...
def main():
    ...
    while not rospy.is_shutdown() and stateManagerInstance.getLoopCount() < 250:
        ...
        if stateManagerInstance.getLoopCount() > 100:  # need to send some position data before we can switch to offboard mode otherwise offboard is rejected
            ...
            # Moving to flight phase 1
            if flightPhase == 0:
                flightPhase = 1
                print("Moving to flight phase: ", flightPhase) # which is take-off
            ...
            if flightPhase == 1:
                ...
                controller.setVel([xGain * (xDesiredDistance - xpos), yGain * (yDesiredDistance - ypos), u],
                              [0, 0, 0])
            ...
            if flightPhase == 2:
                controller.setVel(
                    [xGain * (xDesiredDistance - xpos), yGain * (yDesiredDistance - ypos), xxxxx],
                    [0, 0, 0])
            ...
            if flightPhase == 3:
                ...
                xcontrol = 0.3 - (0.2 / 0.1) * numpy.clip(abs(range1 - zHeight), 0, 0.1)  # this line is unstable when x0 > 0.4, at least on VM workstation (use player!)
